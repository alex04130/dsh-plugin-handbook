...
类型文件上还写着这个方法，动手却炸了：这份按条告诉你还能不能用。闸的全文在 `./gates.md`。
...
- **限定**：读走 CAP-001（可用）。追加需写租约且 `access="write"`。旧服务级 append/inspect 已消失，这不构成整条降级。
...
- **当前状态**：**未核实**（本次盘点 2026-09-13 未调）。服务真名 **`sessionController`**（`ctx.get('sessionController')` / `ctx.remote.session` 生成面）。独立观察：走同一道格式闸（v2 描述符同样炸）。
...
- **当前状态**：**可用**（附 GATE-003）。运行时不拒 `'append'`：当 `'write'`，append 成功。类型域从来没有这个值，所以 TS 能拦、JS 不能。别和 GATE-001 混：那条拒的是 read 句柄上的 append，本条是入参无校验。
...
- **restrict 不是渐进披露**：被摘的名字在该 agent 上直接消失，经 `tool_router` 也是 `unknown tool`（一次工具面收窄实测）。要收窄，在注册期做：预设行，或 `agent.ctx.tools.register` 的 scoped 注册（CAP-032 / 本条本层豁免）。GATE-021 卡全局、GATE-020 卡点名 run_code。

## CAP-021 · 一个会话能调另一个会话的工具吗

- **是什么**：`tool_router` 元工具，`action=call` 转发到内层工具，审批按内层算。
- **实现位置**：`tool_router`（list / describe / call）。注册包路径未在合并时重核。
- **当前状态**：**可用**。
...
- **证据**：<team-preset> `agent.cordis.yml:213-216` `mode: ptc`；`collapses()` 对非 run_code 的顶层调用为真。PTC 会话直调 bash 得到 `unknown tool "bash"`。
...
- **当前状态**：**可用**（类型 + spawn 路径依赖；本次盘点 2026-09-13 未对 `agents.create` 单独打探针）。
...
- **当前状态**：**从未可用**（对该 preset：没装官方 tool-subagent，不是封死）。换模型的官方 UI（GATE-031）在没加那一行之前无从谈起。
- **证据**：对 <team-preset> 目录 grep `tool-subagent` 只打到注释。该 preset 工具清单没有 `subagent` / `list_subagent_models`，有 `<spawn-tool>`。
- **关联**：CAP-040、CAP-033、GATE-031。
- **截至 2026-09-13 · DSH 0.1.5-rc.2**
- **一句话**：没挂。该 preset 换子代理模型只走 `<spawn-tool>`。
...
- **实现位置**：官方工具包 `dsh-tool-skill`（工具清单里已有 `skill`，description 是「Load the full instructions for an available skill」）。管理面是 `skill_list` 等，现仍分名。
...
- **证据**：模块头。本次盘点 2026-09-13 没打空 answerer 探针，outcome 字符串以类型为准。
- **关联**：CAP-033。
- **截至 2026-09-13 · DSH 0.1.5-rc.2**
- **一句话**：没人接就关，不会当默许。
...
- **范围**：本条只钉「子会话内部调宽=封死」。沙箱三档（read-only / workspace-write / danger-full-access）叠加审批的源码坐标未逐条核实。
- **关联**：GATE-032、CAP-033。
- **截至 2026-09-13 · DSH 0.1.5-rc.2**
...
- **类型**：`dsh-agent/lib/types/runtime-types.d.ts:298`，`this: Scoped<Agent>`。来源 `SessionStartSource = 'startup' | 'resume' | 'clear' | 'compact'`（`:105`），四种都触发。
- **关联**：CAP-021。
- **截至 2026-09-13 · DSH 0.1.5-rc.2**
...
- **范围**：本条只证 `tools.guard`。钩子全集未核实。`agent/pre-step`（`runtime-types.d.ts:303`，可拒步/改消息）签名在类型文件，行为未实测。
- **关联**：CAP-023、CAP-080。
- **截至 2026-09-13 · DSH 0.1.5-rc.2**
